...
public class DateUtil {
	 ...
	 * @throws java.text.ParseException the parse exception
	 */
	public static String getLunaDateString(String date) throws ParseException {
		checkSdf();
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(sdf.parse(date));
    ...
    public static Date getLunaDate(String date) throws ParseException {
        checkSdf();
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(sdf.parse(date));
        return LunaDateUtil.getLunal(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.DAY_OF_MONTH)).getTime();
    }
